...
<div class="wrap">
    ...
    <div class="tab-content">
        <div id="tab-1" class="tab-pane <?php echo !isset($_POST["edit_taxonomy"]) ? 'active' : '' ?>">
            ...
            $options = get_option('eugene_plugin_tax') ?: array();

            echo "<table class='cpt-table'><tr><th>ID</th><th>Singular Name</th>
            <th class='text-center'>Hierarchical</th><th class='text-center'>Actions</th></tr>";

            foreach ($options as $option) {

                $hierarchical = isset($option['hierarchical']) ? 'Yes' : 'No';

                echo "<tr><td>{$option['taxonomy']}</td><td>{$option['singular_name']}</td>
                <td class='text-center'>{$hierarchical}</td><td class='text-center'>";
                ...
                settings_fields('eugene_plugin_tax_settings');
        ...
        <div id="tab-2" class="tab-pane <?php echo isset($_POST["edit_taxonomy"]) ? 'active' : '' ?>">
            ...
            <form method="post" action="options.php">
                <?php
                settings_fields('eugene_plugin_tax_settings');
                do_settings_sections('eugene_taxonomy');
                submit_button();
                ?>
            </form>
        </div>
