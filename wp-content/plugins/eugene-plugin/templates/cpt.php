<div class="wrap">
    ...
    <div class="tab-content">
        <div id="tab-1" class="tab-pane <?php echo !isset($_POST["edit_post"]) ? 'active' : '' ?>">
            ...
            foreach ($options as $option) {
                ...
                echo "<tr><td>{$option['post_type']}</td><td>{$option['singular_name']}</td>
                <td>{$option['plural_name']}</td><td class='text-center'>{$public}</td>
                ...
                settings_fields('eugene_plugin_cpt_settings');
        ...
        <div id="tab-2" class="tab-pane <?php echo isset($_POST["edit_post"]) ? 'active' : '' ?>">
            ...
            <form method="post" action="options.php">
                <?php
                settings_fields('eugene_plugin_cpt_settings');
                do_settings_sections('eugene_cpt');
                submit_button();
                ?>
            </form>
        </div>
    ...
    function custom_post_type() {
        ...
        $labels = array(
            ...
            'archives'              => __( '<?php echo $option['singular_name'] . ' Archive'; ?>', 'text_domain' ),
            ...
            'not_found'             => __( '<?php echo 'No ' . $option['singular_name'] . ' Found'; ?>', 'text_domain' ),
            'not_found_in_trash'    => __( '<?php echo 'No ' . $option['singular_name'] . ' Found in Trash'; ?>', 'text_domain' ),
            'featured_image'        => __( 'Featured Image', 'text_domain' ),
            'set_featured_image'    => __( 'Set featured image', 'text_domain' ),
            'remove_featured_image' => __( 'Remove featured image', 'text_domain' ),
            'use_featured_image'    => __( 'Use as featured image', 'text_domain' ),
            'insert_into_item'      => __( '<?php echo 'Insert into ' . $option['singular_name']; ?>', 'text_domain' ),
            'uploaded_to_this_item' => __( '<?php echo 'Upload to this ' . $option['singular_name']; ?>', 'text_domain' ),
